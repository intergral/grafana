...
// opaquePeer implements notify.ClusterPeer but cannot report cluster membership
// (it is neither a *cluster.Peer nor has a Members() method) — like notifier.NilPeer.
type opaquePeer struct{}

func (opaquePeer) Position() int                       { return 0 }
func (opaquePeer) WaitReady(ctx context.Context) error { return nil }
func (opaquePeer) AddState(key string, state cluster.State, reg prometheus.Registerer) cluster.ClusterChannel {
	return nil
}
...
func TestNewPartitionFilter_UnknownPeerType(t *testing.T) {
	// A peer that cannot report membership (e.g. notifier.NilPeer when HA clustering is
	// not configured) must not produce a partitioner.
	filter := NewPartitionFilter(opaquePeer{}, 2)
	assert.Nil(t, filter, "should return nil filter for a peer that cannot report membership")
}
...
func TestPartitioner_PositionExceedsClusterSize(t *testing.T) {
	// The Redis peer's Position() falls back to 0 on lookup failure, and during churn a
	// peer can briefly report a position outside the current member count. Partitioning
	// by a position that does not fit the cluster view would leave shards unevaluated,
	// so the partitioner must fall back to evaluating everything.
	peer := &mockPeer{
		position: 3,
	...
	filtered := filter.Filter(rules)
	assert.Equal(t, len(rules), len(filtered), "out-of-range position should return all rules")

	for _, r := range rules {
		assert.True(t, filter.Owns(r.OrgID, r.UID), "out-of-range position should own all rules")
	}
}

func TestPartitioner_Owns_MatchesFilter(t *testing.T) {
	// Owns and Filter must implement the same predicate: for every peer position, the
	// set of rules Filter returns is exactly the set of rules Owns reports as local.
	members := memberNames(3)
	rules := generateRules(200)

	for pos := 0; pos < 3; pos++ {
		peer := &mockPeer{position: pos, members: members}
		filter := NewPartitionFilter(peer, 2)
		require.NotNil(t, filter)

		owned := make(map[string]bool)
		for _, r := range filter.Filter(rules) {
			owned[r.UID] = true
		}

		for _, r := range rules {
			assert.Equal(t, owned[r.UID], filter.Owns(r.OrgID, r.UID),
				"Owns and Filter disagree for rule %s at position %d", r.UID, pos)
		}
	}
}
...
func TestPartitioner_OrgIsolation(t *testing.T) {
	members := []string{"peer0", "peer1"}
	// minClusterSize of 1 is clamped to 2, which the 2-member cluster still satisfies.
	minClusterSize := 1
...
func TestPartitioner_RapidTopologyChanges(t *testing.T) {
	rules := generateRules(200)
	minClusterSize := 1 // clamped to 2; the 1-member phase falls back to evaluate-all
